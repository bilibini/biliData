...
pool = PooledDB(pymysql,4,host=config['mysql_host'],user=config['mysql_user'],passwd=config['mysql_pw'],database=config['mysql_db'],port=config['mysql_port'],charset='utf8') #5为连接池里的最少连接数
...
class Mydb:
    ...
    def create_table_up(self):
        ...
        CREATE TABLE up (
            ...
            audio smallint unsigned comment '投稿音频数',
            pugv smallint unsigned comment '投稿课程数',
            videolike bigint unsigned comment '视频总点赞量',
            replyemo float(5,4) comment '评论总情绪：0~1，越大越正向',
            danmakuemo float(5,4) comment '弹幕总情绪：0~1，越大越正向',
    ...
    def create_table_login(self):
        ...
        CREATE TABLE login(
            ...
            mid varchar(20) comment 'b站mid',
            token char(70) comment 'token',
            PRIMARY KEY (`id`)
        )
        """
        cursor.execute(sqlstr)
    ...
    def insert_into(self,tab_name:str,data_dict:dict):
        ...
        while init_connect_count<retry_count:
            try:
                ...
                print(f'[数据库操作]{tab_name}表写入错误{init_connect_count}次')
    ...
    def select_table(self,tab_name:str,data_dict:dict={})->list:
        ...
        while init_connect_count<retry_count:
            try:
                ...
                init_connect_count+=1
                print(f'[数据库操作]{tab_name}表查询错误{init_connect_count}次')
        # print(res)
